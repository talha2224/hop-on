...
const insurance = () => {
    ...
    useEffect(() => {
        const loadImages = async () => {
            const savedFiles = await AsyncStorage.getItem("insurance");
            if (savedFiles) {
    ...
    const pickFile = async (index) => {
        ...
        if (!result.canceled) {
            ...
            await AsyncStorage.setItem("insurance", JSON.stringify(updatedFiles));
        }
    };
    ...
    return (
        <View style={nameStyle.container}>
            ...
            <Text style={nameStyle.txt}>Vehicle Insurance Information</Text>
            <Text style={{ color: "#71757b", marginTop: 2 }}>Upload clear and vivid insurance photos</Text>
